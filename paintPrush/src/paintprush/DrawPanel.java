    ...
    private void formMousePressed(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_formMousePressed
        ...
        if (toolPanel.isDrawing()) {
            ...
            if (currentShape instanceof Pencil) {
                ((Pencil) currentShape).last = evt.getPoint();
            }
...
//        System.out.println(last);
        ...
        if (toolPanel.isDrawing()) {
            ...
            if (currentShape instanceof Pencil) {
                for (int i = 0; i < 1; i++) {
                    try {
                        ((Pencil) currentShape).current = evt.getPoint();
//                        ((Pencil) currentShape).allCircles.add((Circle) ((Pencil) currentShape).circle.clone());
                        ((Pencil) currentShape).allCircles.add((Line) ((Pencil) currentShape).circle.clone());

                        ((Pencil) currentShape).allCircles.get(((Pencil) currentShape).allCircles.size() - 1).setP1(((Pencil) currentShape).current);
                        ((Pencil) currentShape).allCircles.get(((Pencil) currentShape).allCircles.size() - 1).setP2(((Pencil) currentShape).last);
                        ((Pencil) currentShape).last = evt.getPoint();
