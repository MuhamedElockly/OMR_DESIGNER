...
public class SmilyFace3 extends Shape {
...
//        g.fillOval(x, y, h, h);
        ...
        g.drawArc(x + (h / 5), y + (2 * h / 4), h - (2 * (h / 5)), h - (2 * (h / 5)), 0, 180);
        g2.setStroke(new BasicStroke(getStroke()));
        g.drawOval(x + ((h / 3)), y + (h / 4), h / 15, h / 15);
        g.drawOval(x + ((2 * h / 3)) - (h / 15), y + (h / 4), h / 15, h / 15);
        g.setColor(Color.BLACK);
        g.fillOval(x + ((h / 3)), y + (h / 4), h / 15, h / 15);
        g.fillOval(x + ((2 * h / 3)) - (h / 15), y + (h / 4), h / 15, h / 15);
...
//        }
    ...
    public String toSVG() {
        ...
        int h = Math.abs(getP1().y - getP2().y);
        String svgString = "";
        svgString = String.format("<circle class=\"smilyFace3\" width=\"%d\" height=\"%d\" cx=\"%d\" cy=\"%d\" r=\"%d\"   fill=\"none\" stroke-width=\"%d\" stroke=\"%s\"/>",
    ...
    public static Shape loadSVG(org.w3c.dom.Node node) {
        SmilyFace3 smilyFace3 = new SmilyFace3();
        ...
        smilyFace3.getP1().x = Integer.parseInt(cxNode.getNodeValue()) - Integer.parseInt(r.getNodeValue());
        smilyFace3.getP1().y = Integer.parseInt(cyNode.getNodeValue()) - Integer.parseInt(r.getNodeValue());
        smilyFace3.getP2().x = Integer.parseInt(cxNode.getNodeValue()) + Integer.parseInt(r.getNodeValue());
        smilyFace3.getP2().y = Integer.parseInt(cyNode.getNodeValue()) + Integer.parseInt(r.getNodeValue());

        if (stroke_width != null && stroke_width.getNodeValue() != null) {
            smilyFace3.strokeColor = Color.decode(stroke.getNodeValue());
            smilyFace3.stroke = Integer.parseInt(stroke_width.getNodeValue());
        } else {
            smilyFace3.nonStroke = true;
        }
